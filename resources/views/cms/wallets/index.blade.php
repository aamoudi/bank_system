...
@section('title', 'Users')
@section('page-name', 'Index Wallets')
              ...
              <table id="table" class="table table-hover table-bordered text-nowrap">
                ...
                <tbody>
                  ...
                  @foreach ($wallets as $wallet)
                    ...
                    <td><span class="badge @if ($wallet->active) bg-success @else
                          bg-danger @endif">{{ $wallet->status }}</span>
                      </td>
                      <td>{{ $wallet->created_at->format('Y-m-d') }}</td>
                      <td>{{ $wallet->updated_at->format('Y-m-d') }}</td>
                      <td>
                        <div class="btn-group">
                          <a href="{{ route('wallets.edit', $wallet->id) }}" type="button" class="btn btn-info"><i
                              class="fas fa-edit"></i></a>
                          <a href="#" class="btn btn-danger" onclick="performDestroy({{ $wallet->id }}, this)"><i
                              class="fas fa-trash"></i></a>
                          <a href="{{ route('transaction.create' ,[ 'wallet_id' =>  $wallet->id ])  }}" type="button" class="btn btn-info"><i class="fas fa-money-check-alt"></i></a>
                        </div>
                      </td>
                    </tr>
                  @endforeach
